...
import parking.shared.parking_rest_helper as resthelper
...
async def test_create_parking_lot_new(http_client, base_url):
    lot = ParkingLot(100, 'test', 1.0, Location(0.0, 1.0))
    plr = resthelper.ParkingLotRest(base_url, http_client)
    response = await plr.set_lot(lot)
